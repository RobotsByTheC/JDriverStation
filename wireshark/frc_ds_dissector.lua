...
function frc_ds.dissector (buf, pkt, root)
	...
	for _, flag in pairs(fields.flags) do
	...
	for _, di in ipairs(fields.digital_input) do
	...
	for j = 1, #fields.joystick do
		local offset = 8 * j
		local joystick_subtree = subtree:add(fields.joystick[j].field, buf(offset, 8))
		for i = 1, #fields.joystick[j].axis do
			joystick_subtree:add(fields.joystick[j].axis[i], buf((offset - 1) + i, 1))
		end
		local joystick_button = buf(offset + 6, 2)
		local joystick_button_subtree = joystick_subtree:add(fields.joystick[j].button_f, joystick_button)
		for _, b in ipairs(fields.joystick[j].button) do
			joystick_button_subtree:add(b, joystick_button)
		end
	end
	for i = 1, #fields.analog_input do
		subtree:add(fields.analog_input[i], buf(38 + 2 * i, 2))
	end
